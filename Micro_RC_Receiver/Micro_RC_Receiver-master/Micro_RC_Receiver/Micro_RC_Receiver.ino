...
#include <SoftwareSerial.h> // used for the YX5300 MP3 player
...
unsigned long millismomentary = millis();
...
byte automateDelay = random(2, 8); // set this to min and max seconds between sounds
...
// ssloan define what pins to use
#define YX5300_RX 0//5  //should connect to TX of the Serial MP3 Player module
#define YX5300_TX 1//6  //connect to RX of the module
...
//static int8_t pre_vol, volume = 0x1e; // Volume. 0-30 DEC values. 0x0f = 15  0x1e=30. 
int volume = 0x1e;
int song = 1;
...
void digitalOutputs() {
    ...
    if ((data.momentary1) && (data.mode1) && (data.mode2) || (data.momentary1) && (!data.mode1) && (!data.mode2)) {
          // MSE Beeps
          if (millis() - millismomentary >= 1000){
//            sendCommand(CMD_PLAY_W_INDEX, random(1,7));   // Alert
//            sendCommand(CMD_PLAY_W_INDEX, random(7,24));  // General
//            sendCommand(CMD_PLAY_W_INDEX, random(24,27)); // Idle
//            sendCommand(CMD_PLAY_W_INDEX, random(27,30)); // Loaded
//            sendCommand(CMD_PLAY_W_INDEX, random(30,32)); // Question
            sendCommand(CMD_PLAY_W_INDEX, random(32,42)); // Run Away
//            sendCommand(CMD_PLAY_W_INDEX, 42); // Mouse Droid Song
//            sendCommand(CMD_PLAY_W_INDEX, 43); // Star Wars
            millismomentary = millis();
            automateDelay = random(7,9);
          }             
    }

    if ((data.momentary1) && (data.mode1) && (!data.mode2)) {
          // MSE Beeps
          if (millis() - millismomentary >= 1000){
            sendCommand(CMD_PLAY_W_INDEX, 43); // Star Wars
            millismomentary = millis();
  ...
  //  When moving forward play "General" sounds at random intervals
  if (((data.mode1) && (data.mode2) && (data.axis3 < 45)) || ((!data.mode1) && (!data.mode2) && (data.axis3 < 45))) {
//    triggerAutomation();
...
//  //  When moving forward play sounds at random intervals
//  if ((data.mode1)&& (data.axis3 < 45)) {
//    triggerAutomation();
//  }

  // Digital Outputs (special functions)
  digitalOutputs();  //Play alarm sounds if momentary "back/pulse" button pressed
